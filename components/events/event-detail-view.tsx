...
/** Published exhibitor counts, for the grid's "Expected" chip. */
    ...
    return (
        <div className="relative isolate min-h-screen space-y-6 max-w-[1600px] mx-auto pb-12">
            ...
            {activeTab === 'Exhibitors' ? (
                ...
                /* The grid fetches per event and renders its own empty state
                   when nothing was imported, so the "not yet available" panel
                   this replaced is no longer needed. */
                <BettExhibitorGrid
                    eventSlug={event.slug}
                    expected={EXPECTED_EXHIBITORS[event.name] ?? null}
                />
            ) : (
