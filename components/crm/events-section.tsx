...
import { findShowEvents, findShowMonthOptions } from "@/lib/find-shows/catalog";
...
function EventListView({ mode = 'all' }: { mode?: 'all' | 'target' }) {
    ...
    const [monthYear, setMonthYear] = useState('All Dates');
    ...
    // Month-Year narrowing (August 2026 ... July 2027) sits outside the rail's
    // filter state: the rail offers rolling presets ("next 3 months"), while the
    // catalog is a fixed 12-month window that people navigate a month at a time.
    // Applied after filterEventList so the two compose.
    const monthMatched = useMemo(
        () => filterEventList(baseEvents, queryState.filters, queryState.search, likedIds),
        [baseEvents, queryState, likedIds]
    );

    const filteredEvents = useMemo(
        () => (monthYear === 'All Dates' ? monthMatched : monthMatched.filter((event) => event.monthYear === monthYear)),
        [monthMatched, monthYear]
    );
    ...
    // A chosen month is a narrowing too, so it also collapses the hero.
    const isSearchActive =
        mode === 'target' || chips.length > 0 || question !== null || monthYear !== 'All Dates';
                    ...
                    <AnimatePresence mode="wait" initial={false}>
                        {isSearchActive ? (
                            ...
                            >
                                ...
                                <div className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-[#22304A]">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {(chips.length > 0 || monthYear !== 'All Dates') && filteredEvents.length > 0 ? (
                                            <button
                                                type="button"
                                                onClick={clearAll}
                            ...
                            >
                                ...
                                {/* Jumping straight to a month is the other way in,
                                    besides asking a question or using the rail. */}
                                <div className="mt-4 flex items-center justify-center gap-2">
                                    <span className="text-[12px] font-semibold text-slate-500 dark:text-[#9CA3AF]">
                                        Or browse by month
                                    </span>
                                    <MonthYearFilterPopover
                                        value={monthYear}
