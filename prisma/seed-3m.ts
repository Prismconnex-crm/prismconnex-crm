...
const PER_CATEGORY = 150000; // 150,000 per category Ã— 20 = 3,000,000
...
async function main() {
  ...
  console.log(`Starting generation of 3,000,000 small businesses (${PER_CATEGORY} per category)...`);
  ...
  for (const category of categories) {
    ...
    // Process in sub-batches of 10,000 to optimize SQLite insert speed and memory usage
    ...
    for (let batch = 0; batch < PER_CATEGORY; batch += SUB_BATCH) {
      ...
      // Insert in chunks of 5000 to prevent SQLite parameter limits (default 32766 parameters)
      const CHUNK = 2000;
  ...
  console.log(`\nSuccessfully seeded 3,000,000 new companies!`);
