...
async function main() {
  ...
  if (!workspace) {
    console.error("Workspace not found. Something went wrong.");
    process.exit(1);
  }
  
  const workspaceId = workspace.id;
  console.log(`Using Workspace ID: ${workspaceId}`);
  console.log("Starting generation of 500,000 small businesses (25,000 per category)...");

  for (const category of categories) {
    console.log(`Generating 25,000 companies for: ${category}`);
    ...
    for (let i = 0; i < 25000; i++) {
      ...
      const name = `${pre} ${mid} ${suf} ${Math.floor(Math.random() * 9999)}`;
    ...
    console.log(`Inserted 25,000 for ${category}.`);
  }
  
  console.log("Successfully seeded 500,000 small businesses!");
}
