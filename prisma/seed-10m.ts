...
const prefixes = ["Omni", "Global", "NextGen", "Pro", "Core", "Prime", "Elite", "Max", "Ultra", "Hyper", "Neo", "Nova", "Zen", "Evo", "Bio", "Eco", "Aero", "Astro", "Cyber", "Tech"];
...
async function main() {
  ...
  console.log("Optimizing SQLite write performance via PRAGMAs for massive 10M seed...");
  await prisma.$queryRawUnsafe('PRAGMA synchronous = OFF;');
  await prisma.$queryRawUnsafe('PRAGMA journal_mode = MEMORY;');
  await prisma.$queryRawUnsafe('PRAGMA temp_store = MEMORY;');

  console.log(`Starting generation of 10,000,000 businesses across all employee ranges (${PER_CATEGORY} per category)...`);
  ...
  for (const category of categories) {
    ...
    for (let batch = 0; batch < PER_CATEGORY; batch += SUB_BATCH) {
      ...
      for (let i = 0; i < currentBatchSize; i++) {
        ...
        // Add index to ensure uniqueness over massive run
        const uniqueId = globalIdx + 1000000; 
  ...
  console.log(`\nSuccessfully seeded 10,000,000 new companies across all ranges!`);
