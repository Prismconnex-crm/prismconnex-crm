 ...
 * password. Signing them in here would skip the sign-in step the UI promises —
 * and confirming the new password actually works is the point.
 *
 * Existing sessions are a separate matter, and both halves are ended here:
 * AuthService revokes every Supabase session (scope=global), and this route
 * expires this browser's auth cookies. Without the latter, middleware.ts would
 * see a still-valid pcx_session and bounce the post-reset redirect from
 * /auth/sign-in straight to /app/dashboard, skipping that confirmation step.
 */
