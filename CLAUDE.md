...
- `npm test` — vitest (tests in `tests/integration`, node environment, `@/` alias = repo root)
...
- Prisma client singleton: `lib/db/prisma.ts` (applies WAL/cache PRAGMAs on startup). `lib/prisma.ts` is an older unused duplicate — do not import it.
...
- `package.json` still carries unused deps from past experiments (`@prisma/adapter-pg`, `pg`).
